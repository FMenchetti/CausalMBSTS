...
mbsts.mcmc<-function(Smodel, X, H = NULL, nu0.k = NULL, s0.k, nu0.eps = NULL, s0.eps, niter, 
                     burn, ping = NULL){
  ...
  # set default ping
  if(is.null(ping)){ping<- 0.1*niter} 
  sequence<-seq(ping,niter,by=ping) 
  
  # set default nu0.k and nu0.eps
  if(is.null(nu0.k)){nu0.k <- p+2}
  if(is.null(nu0.eps)){nu0.eps<-p+2}
  ...
  for(i in 1:niter){
    ...
    for(j in sample(1:dim(X)[2])){
      zp<-z 
      zp[j]<-1-zp[j]
      lpy.p<-lpy.X(y.star,X[,zp==1,drop=F], H = H[zp==1,zp==1,drop=F], nu0.eps = nu0.eps, s0.eps = s0.eps)
  ...
  list<-list(eta.samples = eta.samples, eps.samples = eps.samples, states.samples = states.samples,
             Sigma.eta = Sigma.states, Sigma.eps = Sigma.obs, Z.beta = Z, beta = BETA, X = X,
             y = y, Z = Smodel$Z , T = Smodel$T, R = Smodel$R, niter=niter, burn=burn)
  class(list)<-'mbsts'
  return(list)
}  
...
lpy.X <-function(y,X,H,s0.eps,nu0.eps){
  ...
  if(dim(X)[2]==0){
    ...
    -(p*t/2)*log(pi) + (nu0.eps/2)*log(det(s0.eps)) + 
      lmvgamma(nu.eps/2,p) - lmvgamma(nu0.eps/2,p) - (nu.eps/2)*log(det(s.eps))
  } else {
    W<-inv(crossprod(X) + inv(H)) 
    M<-tcrossprod(W,X) %*%y 
    nu.eps<-nu0.eps + t
    s.eps<-s0.eps + crossprod(y) - crossprod(M,solve(W,M))
    (p/2)*log(det(inv(H)%*%W)) -(p*t/2)*log(pi) + (nu0.eps/2)*log(det(s0.eps)) + 
      lmvgamma(nu.eps/2,p) - lmvgamma(nu0.eps/2,p) - (nu.eps/2)*log(det(s.eps))
  }
}
